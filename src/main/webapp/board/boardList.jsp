				...
				<div>
					...
					BoardService bs = BoardService.getInstance();
								...
								*/
								...
								int startNum = bs.startNum(pageScale, currentPage); //pagenation을 클릭하면 해당페이지의 번호로 설정
					...
					현재 페이지 pagination번호 : ${currentPage }<br>
					시작번호 : ${startNum }<br>
					끝번호 : ${endNum }<br> --%>
					
					<%-- 총 <c:out value="${totalPage}"/> 페이지 중 현재 <c:out value="${currentPage}"/> 페이지 입니다. --%> 
					...
					<div id="boardSearchDiv" style="text-align:center">
						<form id="boardSearchFrm" action="boardList.jsp">
