...
<%
int cnt = SelectService4.getInstance().dynamicSet(eDTO);
String msg=eDTO.getEmpno()+"번";
if(cnt == 1){
	msg+=" 사원 정보 변경 성공";
}else{
	msg+=" 사원 정보 변경되지 않았습니다.";
}//end else
...
<form id="updateFrm" name="updateFrm" action="index.jsp">
<input type="hidden" name="url" value="${param.url}"/>
...
<input type="button" value="변경" class="btn btn-sm btn-info" id="btnModify"/>
</form>
</c:when>
<c:otherwise>
해당 사원은 존재하지 않습니다.
